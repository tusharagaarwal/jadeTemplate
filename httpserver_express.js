...
function login(req, res){
	console.log(Date() + ": Request for Login from: " + req.connection.remoteAddress);
...
app.get('/home.jade', function(req, res){
	...
	res.render('home', {locals: {data: {name: 'tushar'}}});
});
...
app.get(['*.css', '*.js'], function(req, res) {
	console.log(Date() + ": Request from the user: " + req.session_state.username + " from " + req.connection.remoteAddress);
	loadFile(url.parse(req.url).pathname, req, res);
});
